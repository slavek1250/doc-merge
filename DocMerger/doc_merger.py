...
class Tools:
    ...
    def create_empty_page(self, src_pdf_file_path : Path, output_pdf_file_path : Path) -> None:
        with open(src_pdf_file_path, 'rb') as pdf:
            ...
            empty_page_canvas = canvas.Canvas(output_pdf_file_path.as_posix(), pagesize=pdf_pagesize)
...
class DocMerger:
    def __init__(self, input_dir_path: Path, output_pdf_path: Path, align: int = 2, progress_log : ProgressLogIf = DummyProgressLog()) -> None:
    ...
    def execute(self):
        ...
        if len(self._pdf_files) == 0 and len(self._nopdf_files) == 0:
            self._progress_log.msg("No files to merge, check if input path is valid and contain supported file types!")
        else:
            self._exec()
    ...
    def _enumerate_files_to_process(self):
        self._nopdf_files = self._nopdf_converter.list_supported_files_in_dir(self._input_dir_path)
    ...
    def _init_progress_log(self):
        ...
        total_operations_num = pdfs_num * OPERATIONS_NUM_PER_PDF + nopdfs_num * OPERATIONS_NUM_PER_NOPDF + SAVE_MERGED_FILE_OPERATIONS_NUM
    ...
    def _exec(self):
        with tempfile.TemporaryDirectory() as aligned_pdfs_dir:
            ...
            if len(self._nopdf_files) > 0:
                self._align_pages_num_nopdf_list(self._nopdf_files, aligned_pdfs_dir_path)
    ...
    def _align_pages_num_nopdf_list(self, nopdf_file_paths : list[Path], output_dir_path: Path):
        with tempfile.TemporaryDirectory() as converted_files_dir:
            ...
            for nopdf_file_path in nopdf_file_paths:
                ...
                converted_file_path = Path(f"{converted_files_dir}/{nopdf_file_path.stem}.pdf")
    ...
    def _align_pages_num_pdf_list(self, pdf_file_paths: list[Path], output_dir_path: Path):
        with tempfile.TemporaryDirectory() as empty_page_dir:
            for pdf_file_path in pdf_file_paths:
                output_pdf_file_path = Path(f"{output_dir_path.as_posix()}/{pdf_file_path.name}")
                ...
                self._align_pages_num_pdf(pdf_file_path, output_pdf_file_path, empty_page_pdf_file_path)
                self._progress_log.next()

    def _align_pages_num_pdf(self, pdf_file_path: Path, output_pdf_file_path : Path, empty_page_pdf_file_path : Path):
        ...
        self._progress_log.msg(f"File has {pages_num} pages, adding {empty_pages_num_to_append} empty page(s): {pdf_file_path.name}")
    ...
    def _merge_pdfs(self, pdfs_input_dir: Path):
        ...
        self._progress_log.msg(f"Creating output PDF file: {self._output_pdf_path.as_posix()}")
        with PdfWriter() as pdf_writer:
